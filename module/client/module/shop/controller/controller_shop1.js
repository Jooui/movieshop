...
function loadItems(type = "default",mode = "alph"){
    ...
    if (idGenreOnLocalStorage == null){ //Si no hay ningun genero en localstorage (DESDE GENERO HOME) | CARGA NORMAL
        urlAjax = "/movieshop/module/client/module/shop/controller/controller_shop.php?op=getMovies";
        localStorage.setItem('genresChecked',null);
    }else{//si hay genero desde home, carga ese genero
        localStorage.setItem('genresChecked',null);
        urlAjax = "/movieshop/module/client/module/shop/controller/controller_shop.php?op=getMoviesFilterGenres";
    }

    if(type == "checkbox"){//Si se han seleccionado opciones de filtro checkbox, lo guarda en localStorage, más adelante carga la url
    ...
    if(type == "alph"||type == "rate"||type == "date"){
        urlAjax = "/movieshop/module/client/module/shop/controller/controller_shop.php?op=getMoviesCheckBox";
    }
